...
export async function updateRentPropertyField(locationId: string, propertyId: string, fields: Record<string, any>): Promise<void> {
...
export async function updateBuyPropertyField(locationId: string, propertyId: string, fields: Record<string, any>): Promise<void> {
...
export async function addAmenityHeading(locationId: string, title: string): Promise<string> {
  const ref = await addDoc(collection(db, 'locations', locationId, 'amenityHeadings'), { title, createdAt: serverTimestamp() });
  return ref.id;
}
...
export async function addAmenityPlace(locationId: string, headingId: string, place: Omit<AmenityPlace, 'id'>): Promise<string> {
  const ref = await addDoc(collection(db, 'locations', locationId, 'amenityHeadings', headingId, 'places'), { ...place, createdAt: serverTimestamp() });
  return ref.id;
}
